...
def build_from_config(config_path: Path) -> None:
    ...
    channels = 1 if flags.num_channels == 0 else 2
    ...
    for (local_idx, (slice_idx, _wav_path)) in enumerate(written):
        ...
        frames = plan.slices[slice_idx - start_index].length
        data_bytes = frames * channels * 2  # PCM16 bytes
        stop_bytes = int(round(data_bytes / 4.0)) * 4

        content = InfoContent(
            size_bytes=stop_bytes,
            flags=flags,
            slice_starts=[0],
            slice_stops=[stop_bytes],
